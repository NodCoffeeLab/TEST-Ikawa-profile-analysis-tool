...
if st.session_state.processed_profiles:
    ...
    with analysis_col:
        ...
        for name in selected_profiles_data:
            ...
            st.markdown(f"<p style='margin-bottom:0; margin-top:-5.0; font-size: 0.9em;'>&nbsp;&nbsp;• 온도: {temp_str}</p>", unsafe_allow_html=True)
            st.markdown(f"<p style='margin-bottom:0; margin-top:0; font-size: 0.9em;'>&nbsp;&nbsp;• ROR: {ror_str}</p>", unsafe_allow_html=True)
            st.markdown(f"<p style='margin-bottom:3.0em; margin-top:0; font-size: 0.9em;'>&nbsp;&nbsp;• 팬: {fan_str}</p>", unsafe_allow_html=True)
